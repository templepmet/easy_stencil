...
class GridSeries {
   ...
   public:
    GridSeries(T *h_buf, ull width, ull height, ull halo) : _h_buf(h_buf), _width(width), _height(height), _halo(halo) {
        // split buf to chunk
        ...
        ull d_work_height = std::min((available_size + (_width * 2 - 1)) / (_width * 2) - _halo * 2, height);
        ...
        while (start_row < height) {
            ...
            _h_chunk_list.push_back(new ChunkHost<T>(h_start_buf, width, chunk_height, halo));
            h_start_buf += sizeof(T) * width * chunk_height;
    ...
    std::vector<ChunkHost<T>*>& getChunkHostList() {
        return _h_chunk_list;
    }
