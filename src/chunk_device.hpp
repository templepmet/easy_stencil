   ...
   public:
    ChunkDevice(ull width, ull height, ull halo)
        ...
        HANDLE_CUDA(cudaMalloc(
            &this->_buf,
            sizeof(T) * this->_width * (this->_height + this->_halo * 2)));
    }

    ~ChunkDevice() { HANDLE_CUDA(cudaFree(this->_buf)); }

    ChunkDevice<T>& operator=(const ChunkDevice<T>& right_chunk) {
        assert(this->_target_size == right_chunk.getTargetSize());
        HANDLE_CUDA(
            cudaMemcpyAsync(this->_target_ptr, right_chunk.getTargetPtr(),
                            this->_target_size, cudaMemcpyDeviceToDevice));
        return *this;
    }
    ...
    ChunkDevice<T>& operator=(const ChunkHost<int>& right_chunk) {
        ...
        HANDLE_CUDA(
            cudaMemcpyAsync(this->_target_ptr, right_chunk.getTargetPtr(),
                            this->_target_size, cudaMemcpyHostToDevice));
        return *this;
    }
