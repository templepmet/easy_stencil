...
int main(int argc, char **argv) {
    ...
    std::vector<ChunkHost<TYPE>*> &h_chunk_list = grid_series.getChunkHostList();
    std::vector<ChunkDevice<TYPE>*> &d_chunk_works = grid_series.getChunkDeviceWorks();
    assert(d_chunk_works.size() == 2);
    int chunkDim = h_chunk_list.size();

    printf("chunk_dim=%d\n", h_chunk_list.size());
    for (Chunk<TYPE>* chunk : h_chunk_list) {
        printf("width=%lld,height=%lld\n", chunk->getWidth(), chunk->getHeight());
    }
    ...
    for (int i = 0; i < loops; ++i) {
        for (int chunkIdx = 0; chunkIdx < chunkDim; ++chunkIdx) {
            ...
            d_chunk_works[step0]->Rows(0, copy_rows) = chunk->Rows(0, copy_rows);
            d_chunk_works[step1]->Rows(-halo + 1, halo) = d_chunk_works[step0]->Rows(chunk_height - halo, halo);
            d_chunk_works[step0]->compute();
            chunk->Rows(0, chunk_height) = d_chunk_works[step0]->Rows(0, chunk_height);
        }
    }
